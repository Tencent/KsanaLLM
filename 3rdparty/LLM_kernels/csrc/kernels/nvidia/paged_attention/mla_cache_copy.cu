...
__global__ void CopyKeyBlockWithReplicationKernel(SCALAR_T* k_out, void** k_list, int src_stride_size,
  ...
  void* k_block_base = reinterpret_cast<void*>(k_list[block_offsets[batch_idx] + block_offset_in_req]);
...
// Drain prefix part of attention output, used to restore hidden unit dimension.
// This kernel will drain
//     [num_total_tokens, num_heads, qk_nope_head_dim+qk_rope_head_dim]
// to
//     [num_total_tokens_without_prefix, num_heads, qk_nope_head_dim+qk_rope_head_dim]
//
// Args:
//   out: The drained output.
//   src: The attention output.
//   prefix_offsets:
//     The offset of prefix token num.
//     It's shape is [batch_size + 1], in format [0, seq1_prefix_len, seq1_prefix_len + seq2_prefix_len, ...]
//   without_prefix_offsets:
...
//   head_size:        The dim of every head.
//   batch_size:       The batch size.
//   total_len_without_prefix: The toeken number of all tokens, prefix part only.
template <typename SCALAR_T, typename CACHE_T, llm_kernels::utils::KVCacheType KV_DTYPE>
__global__ void MlaDrainAttnOutPrefixKernel(SCALAR_T* out, SCALAR_T* input, size_t* prefix_offsets,
                                            size_t* without_prefix_offsets, int num_heads, int head_size,
                                            int batch_size, int total_len_without_prefix) {
  int token_idx_without_prefix = blockIdx.y + blockIdx.z * gridDim.y;
  if (token_idx_without_prefix >= total_len_without_prefix) {
    return;
  }

  int batch_idx = 0;
  for (batch_idx = 0; batch_idx < batch_size; batch_idx++) {
    if (token_idx_without_prefix < without_prefix_offsets[batch_idx + 1]) {
      break;
    }
  }

  int token_stride_size = num_heads * head_size;
  int total_token_idx = prefix_offsets[batch_idx + 1] + token_idx_without_prefix;

  // Process every head
  for (int head_idx = blockIdx.x; head_idx < num_heads; head_idx += gridDim.x) {
    for (int head_dim_idx = threadIdx.x; head_dim_idx < head_size; head_dim_idx += blockDim.x) {
      int src_offset = (total_token_idx * token_stride_size) + (head_idx * head_size) + (head_dim_idx);
      int dst_offset = (token_idx_without_prefix * token_stride_size) + (head_idx * head_size) + head_dim_idx;
      out[dst_offset] = input[src_offset];
    }
  }
}
...
void MlaDrainAttnOutPrefix(SCALAR_T* out, SCALAR_T* input, size_t* prefix_offsets, size_t* without_prefix_offsets,
                           ...
                           cudaStream_t stream) {
  int grid_y = std::min(total_len_without_prefix, MAX_BLOCKS_PER_GRID_Y);
  int grid_z = (total_len_without_prefix + MAX_BLOCKS_PER_GRID_Y - 1) / MAX_BLOCKS_PER_GRID_Y;
  dim3 grid_shape(1, grid_y, grid_z);

  dim3 block_shape(std::min(batch_size, MAX_THREADS_PER_BLOCK));
  MlaDrainAttnOutPrefixKernel<SCALAR_T, CACHE_T, KV_DTYPE><<<grid_shape, block_shape, 0, stream>>>(
      out, input, prefix_offsets, without_prefix_offsets, num_heads, head_size, batch_size, total_len_without_prefix);
}
...
#define MLA_CACHE_COPY_FUNCTION_DECLARATION(SCALAR_T, CACHE_T, KV_DTYPE)                                              \
  ...
  template void MlaFillKVPrefix<SCALAR_T, CACHE_T, KV_DTYPE>(                                                         \
      ...
      cudaStream_t stream);                                                                                           \
  template void MlaDrainAttnOutPrefix<SCALAR_T, CACHE_T, KV_DTYPE>(                                                   \
      SCALAR_T * out, SCALAR_T * input, size_t* prefix_offsets, size_t* without_prefix_offsets, int num_heads,        \
      int head_size, int batch_size, int total_len_without_prefix, cudaStream_t stream);
