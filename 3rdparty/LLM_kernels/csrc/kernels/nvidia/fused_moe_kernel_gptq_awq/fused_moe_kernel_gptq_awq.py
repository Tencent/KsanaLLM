...
def performance_fused_moe_gptq_awq(
        A: torch.Tensor,
        B: torch.Tensor,
        C: torch.Tensor,
        B_scale: Optional[torch.Tensor],
    ...
    output_tensor, kernel = fused_moe_gptq_awq(A, B, C, B_scale, B_zp,
                                               topk_weights, topk_ids, sorted_token_ids,
                                               expert_ids, num_tokens_post_padded,
                                               mul_routed_weight, topk,
                                               config, compute_type, has_zp,
                                               use_int8_w8a16, use_int4_w4a16, block_shape)
    torch.cuda.synchronize()
    graph = torch.cuda.CUDAGraph()
    with torch.cuda.graph(graph):
        for _ in range(10):
            fused_moe_gptq_awq(A, B, C, B_scale, B_zp,
                               topk_weights, topk_ids, sorted_token_ids,
                               expert_ids, num_tokens_post_padded,
                               mul_routed_weight, topk,
                               config, compute_type, has_zp,
...
if __name__ == "__main__":
    ...
    default_kernel, kernel_time, output_tensor = performance_fused_moe_gptq_awq(
            A, B, C, B_scale, B_zp,
    ...
    for num_warps in [4, 8]:
        for num_stages in [2, 3, 4, 5]:
            ...
            kernel, kernel_time, output_tensor = performance_fused_moe_gptq_awq(
                A, B, C, B_scale, B_zp,
                topk_weights, topk_ids, sorted_token_ids, expert_ids, num_tokens_post_padded,
                mul_routed_weight, topk,
