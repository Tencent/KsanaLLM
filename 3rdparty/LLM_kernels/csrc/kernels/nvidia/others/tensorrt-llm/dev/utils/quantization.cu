...
void invokeFP4Quantization(int m, int n, T const* input, float const* SFScale, int64_t* output, int32_t* SFOuput,
                           bool useUE8M0, FP4QuantizationSFLayout layout, int multiProcessorCount,
                           cudaStream_t stream) {
#ifdef ENABLE_FP8
...
#endif
  {
    ...
    // Launch the cvt kernel.
    auto* kernel_instance = useUE8M0 ? &cvt_fp16_to_fp4<T, SF_VEC_SIZE, true> : &cvt_fp16_to_fp4<T, SF_VEC_SIZE, false>;
...
void invokeBatchedFP4Quantization(int b, int m, int n, T const* input, float const* SFScale, int64_t* output,
  ...
  if constexpr (std::is_same_v<T, __nv_fp8_e4m3>) {
    ...
    // Launch the cvt kernel.
    auto* kernel_instance = useUE8M0 ? &cvt_fp8_to_fp4_3d<SF_VEC_SIZE, true> : &cvt_fp8_to_fp4_3d<SF_VEC_SIZE, false>;
  ...
  for (int rowIdx = blockIdx.x; rowIdx < numRowsPadded; rowIdx += gridDim.x) {
    for (int batchIdx = 0; batchIdx < numBatches; batchIdx++) {
      for (int colIdx = threadIdx.x; colIdx < numColsPadded; colIdx += blockDim.x) {
        ...
        auto dstIdx =
            get_sf_out_offset_128x4<SF_VEC_SIZE>(batchIdxOpt, rowIdx, colIdx, numRowsOpt, numCols * SF_VEC_SIZE);
        SFOutput[dstIdx] = sf;
      }
    }
  }
...
__global__ void nvfp4_block_scale_interleave_reverse_kernel(int numBatches, int numRows, int numCols,
...
// This is intended for weight loading, so m and n are large, b <= 256
void invokeNVFP4BlockScaleInterleaveReverse(int b, int m, int n, uint8_t const* SFIn, uint8_t* SFOutput,
