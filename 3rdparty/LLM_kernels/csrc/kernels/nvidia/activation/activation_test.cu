...
TEST_F(LlamaNvidiaActivationTestSuit, HalfActivationCommonTest) {
  TestActivation<SiluActivation, half>(stream);
  TestActivation<GeluActivation, half>(stream);
  TestActivation<ReluActivation, half>(stream);
}

TEST_F(LlamaNvidiaActivationTestSuit, FloatActivationCommonTest) {
  TestActivation<SiluActivation, float>(stream);
  TestActivation<GeluActivation, float>(stream);
  TestActivation<ReluActivation, float>(stream);
}

TEST_F(LlamaNvidiaActivationTestSuit, Bf16ActivationCommonTest) {
  TestActivation<SiluActivation, __nv_bfloat16>(stream);
  TestActivation<GeluActivation, __nv_bfloat16>(stream);
  TestActivation<ReluActivation, __nv_bfloat16>(stream);
}
