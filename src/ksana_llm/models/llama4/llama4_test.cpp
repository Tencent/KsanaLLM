...
class Llama4Test : public testing::Test {
 protected:
  void SetUp() override {
    ...
    env->ParseModelConfig("/model/Llama-4-Scout-layer2", "/model/Llama-4-Scout-layer2");
 ...
 protected:
  ...
  void TestLlama4Forward() {
    ...
    std::shared_ptr<BaseWeight> llama4_weight =
        std::make_shared<Llama4Weight<weight_data_type>>(model_config, 0, context_);
    // Start Loader Weight
    ...
    for (std::string &file_name : weights_file_list) {
      ...
      llama4_weight->LoadWeightsFromFile(weights_loader, weight_name_list, custom_name_list);
      StreamSynchronize(context_->GetMemoryManageStreams()[device_id]);
    }
    llama4_weight->ProcessWeights();  // End Loader Weight
    std::shared_ptr<Llama4Model<weight_data_type>> llama4 =
        std::make_shared<Llama4Model<weight_data_type>>(model_config, 0, context_, llama4_weight);
    llama4->AllocResources(schedule_id);

    // ContextDecode
    ForwardRequest forward;
    forward.cache_manager = cache_manager;
    std::vector<int> input_ids = {200000, 62, 19441, 583, 4242, 24};
    ...
    forward.logits_buf[0] = llama4->GetLogitsPtr(schedule_id);
    ...
    for (int i = 0; i < rounds; ++i) {
      llama4->Forward(schedule_id, llama4_weight, multi_forward_reqs, false);
    }
