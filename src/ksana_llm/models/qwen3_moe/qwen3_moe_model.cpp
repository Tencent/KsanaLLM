...
template <typename T>
Qwen3MoeDecoderLayer<T>::Qwen3MoeDecoderLayer(int layer_idx, TensorBuffer* moe_buffer,
...
template <typename T>
Status Qwen3Moe<T>::GetModelRunConfig(ModelRunConfig& model_run_config, const ModelConfig& model_config) {
...
template <typename T>
Status Qwen3Moe<T>::CreateLayers(LayerCreationContext& creation_context, ModelCreationConfig& model_creation_config) {
...
template <typename T>
Status Qwen3Moe<T>::Forward(std::vector<Tensor>& residual_buffer, ForwardingContext& forwarding_context) {
...
template <typename T>
Status Qwen3MoeModel<T>::CreateLayers(LayerCreationContext& creation_context,
                                      ModelCreationConfig& model_creation_config) {
...
template <typename T>
Status Qwen3MoeModel<T>::LayerForward(ForwardingContext& forwarding_context, const RunMode run_mode) {
