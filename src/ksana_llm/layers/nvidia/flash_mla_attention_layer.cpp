...
void MlaAttenVarlen(void* output_buffer, void* q_nope_ptr, void* q_pe_ptr, void* k_pe_ptr, void* compressed_kv_ptr,
                    ...
                    int stride_size, float k_scale, float v_scale, bool is_causal, int rank, int block_size,
                    void** k_list, void** v_list, void* prefix_offsets, void* block_offsets,
                    const std::optional<void*>& alibi_slopes, void* flexible_rotary_embedding_pos_ptr,
                    ...
                    void* flexible_offset_uint64_ptr, int flexible_len, void* q_norm_weight, void* k_norm_weight,
                    bool use_cache, cudaStream_t stream, void* k_cache_ptr, void* v_cache_ptr, int32_t* block_table_ptr,
                    int64_t kv_cache_block_num, int max_blocks_per_seq, size_t* without_prefix_offsets,
                    int max_forwarding_tokens, int total_prefix_len, void* seqlens_q_ptr, void* prefix_k_buffer,
                    void* prefix_v_buffer, void* prefix_kv_buffer, void* prefix_k_up_buffer, void* prefix_v_up_buffer,
  ...
  const auto int_options = torch::TensorOptions().device(torch::kCUDA, rank).dtype(torch::kInt64);
...
#define MLA_ATTEN_VARLEN(SCALAR_T, CACHE_T, KV_DTYPE)                                                                 \
  template void MlaAttenVarlen<SCALAR_T, CACHE_T, KV_DTYPE>(                                                          \
      ...
      int v_head_dim, int num_kv_heads, int head_size, int stride_size, float k_scale, float v_scale, bool is_causal, \
      int rank, int block_size, void** k_list, void** v_list, void* prefix_offsets, void* block_offsets,              \
      const std::optional<void*>& alibi_slopes, void* flexible_rotary_embedding_pos_ptr,                              \
      void* flexible_rotary_embedding_mask_ptr, void* dst_flexible_kv_cache_ptr, void* src_flexible_kv_cache_ptr,     \
      void* dst_flexible_token_idx_ptr, void* src_flexible_token_idx_ptr, void* flexible_offset_uint64_ptr,           \
      int flexible_len, void* q_norm_weight, void* k_norm_weight, bool use_cache, cudaStream_t stream,                \
      ...
      void* seqlens_q_ptr, void* prefix_k_buffer, void* prefix_v_buffer, void* prefix_kv_buffer,                      \
      void* prefix_k_up_buffer, void* prefix_v_up_buffer, QuantMode mm_quant_mode)
...
Status FlashMlaAttentionLayer<SCALAR_T, CACHE_T, KV_DTYPE>::Forward(const std::vector<Tensor>& input_tensors,
  ...
  const Tensor& dp_flexible_rotary_embedding_pos = *input_iter++;
  const Tensor& dp_flexible_rotary_embedding_mask = *input_iter++;
  ...
  const Tensor& query_layernorm_weight = *input_iter++;
  const Tensor& key_layernorm_weight = *input_iter++;
  const Tensor& use_cache_flag = *input_iter++;
  const Tensor& layer_kv_cache = *input_iter++;
  const Tensor& block_table = *input_iter++;
  const Tensor& dp_input_without_prefix = *input_iter++;
  ...
  const bool use_cache = use_cache_flag.GetPtr<bool>()[0];
  ...
  MlaAttenVarlen<SCALAR_T, CACHE_T, KV_DTYPE>(
      ...
      this->is_causal_, this->rank_, this->block_token_num_, k_list, v_list, dp_input_prefix.GetPtr<void>(),
      kv_cache_offset.GetPtr<void>(), this->alibi_slopes_, dp_flexible_rotary_embedding_pos.GetPtr<void>(),
      dp_flexible_rotary_embedding_mask.GetPtr<void>(), dp_dst_flexible_kv_cache.GetPtr<void>(),
      dp_src_flexible_kv_cache.GetPtr<void>(), dp_dst_flexible_token_idx.GetPtr<void>(),
      dp_src_flexible_token_idx.GetPtr<void>(), dp_flexible_offset_uint64.GetPtr<void>(),
      dp_dst_flexible_kv_cache.shape[0], query_layernorm_weight.GetPtr<void>(), key_layernorm_weight.GetPtr<void>(),
      use_cache, this->context_->GetComputeStreams()[this->rank_].Get(), k_cache_ptr, v_cache_ptr,
      block_table.GetPtr<int32_t>(), kv_cache_block_num, max_blocks_per_seq, dp_input_without_prefix.GetPtr<size_t>(),
      max_forwarding_tokens, total_prefix_len, dp_prefill_q_offset.GetPtr<void>(), prefix_k_buffer.GetPtr<void>(),
      prefix_v_buffer.GetPtr<void>(), prefix_kv_buffer.GetPtr<void>(), prefix_k_up_buffer.GetPtr<void>(),
      prefix_v_up_buffer.GetPtr<void>(), this->mm_quant_mode_);
